...
if(isset($_POST['image_profil'])){
    if ($_FILES['image']['error'] > 0) { $erreur=0; header('Location: ../vue/profil_V.php?erreur='.$erreur);}
    if ($_FILES['image']['size'] > $maxsize){ $erreur=1; header('Location: ../vue/profil_V.php?erreur='.$erreur);}
    $extensions_valides = array( 'jpg' , 'jpeg' , 'gif' , 'png' );
    //1. strrchr renvoie l'extension avec le point (« . »).
    //2. substr(chaine,1) ignore le premier caractère de chaine.
    //3. strtolower met l'extension en minuscules.
    $extension_upload = strtolower(  substr(  strrchr($_FILES['image']['name'], '.')  ,1)  );
    if (!in_array($extension_upload,$extensions_valides) ){ $erreur=2; header('Location: ../vue/profil_V.php?erreur='.$erreur);}
    else{
...
if(isset($_POST['modifier'])){
    ...
    $req->execute(array(
        ...
        'pseudo' => $_POST['pseudo_m'], 
        'date' => $_POST['date_m'],
        'adresse' => $_POST['adresse_m'], 
        'code_postal' => $_POST['code_postal_m'],
        'ville' => $_POST['ville_m'], 
        'pays' => $_POST['pays_m'],
	'id' => $_SESSION['id'])
	);
